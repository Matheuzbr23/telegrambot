...
public class MotorRespostas {
    ...
    public SendMessage answerMessage() {
		...
		if ((conversa.getStatus() == ConversaStatus.NOVA || conversa.getStatus() == ConversaStatus.COMPLETO ) && respostaUsuario.toLowerCase().contains("oi ")) {
			message.setText("Oi!");
		...
		else if ((conversa.getStatus() == ConversaStatus.NOVA || conversa.getStatus() == ConversaStatus.COMPLETO ) && matcherTudoBem.find()){
		...
		if ((conversa.getStatus() == ConversaStatus.NOVA || conversa.getStatus() == ConversaStatus.COMPLETO ) && (respostaUsuario.contains("previsao") || (respostaUsuario.contains("previsão")))) {

			conversa.setStatus(ConversaStatus.PREVISAO_REQUISITADA);

            message.setReplyMarkup(RespostaKeyboard.getYesNoKeyboard());
		
			message.setText("Você gostaria de saber qual é a previsão do tempo ?");
			message.setChatId(chatID);
			return message;
		}
		...
		if (conversa.getStatus() == ConversaStatus.PREVISAO_REQUISITADA && respostaUsuario.contains("sim")) {
			...
			message.setText("pq faz içu cmg :(");
			message.setChatId(chatID);
			return message;
		}
